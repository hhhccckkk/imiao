...
public abstract class UDPClientBase implements Runnable {
	...
	public UDPClientBase(byte[] uuid, int appid, String serverAddr,
	...
	public void run() {
			...
			try {
				if (hasNetworkConnection() == false) {
					try {
						...
						LogUtil.E("onConnectionFail 000: "+e.toString());
				...
				onConnectionFail(e.toString());
				LogUtil.E("onConnectionFail 111: "+e.toString());
			} catch (Exception e) {
				e.printStackTrace();
				this.needReset = true;
				onConnectionFail(e.toString());
				LogUtil.E("onConnectionFail 222: "+e.toString());
			} catch (Throwable t) {
				t.printStackTrace();
				this.needReset = true;
				onConnectionFail(t.toString());
				LogUtil.E("onConnectionFail 333: "+t.toString());
			} finally {
				if (needReset == true) {
					try {
						trySystemSleep();
						...
						onConnectionFail(e.toString());
						LogUtil.E("onConnectionFail 444: "+e.toString());
					}
				}
				if (mq.isEmpty() == true || hasNetworkConnection() == false) {
					try {
						...
						onConnectionFail(e.toString());
						LogUtil.E("onConnectionFail: 555"+e.toString());
					}
				}
			}
		}
		if (ds != null) {
			try {
				...
				onConnectionFail(e.toString());
				LogUtil.E("onConnectionFail 6666: "+e.toString());
			}
			ds = null;
		}
	}
	...
	private void ackServer(Message m) throws Exception {
		if (m.getCmd() == Message.CMD_0x10) {
			byte[] buffer = new byte[Message.CLIENT_MESSAGE_MIN_LENGTH];
			ByteBuffer.wrap(buffer).put((byte) Message.version)
					.put((byte) appid).put((byte) Message.CMD_0x10).put(uuid)
					.putChar((char) 0);
			send(buffer);
		}
		if (m.getCmd() == Message.CMD_0x11) {
			...
			byte[] data = m.getData();
			ByteBuffer.wrap(buffer).put((byte) Message.version)
					.put((byte) appid).put((byte) Message.CMD_0x11).put(uuid)
					.putChar((char) 8)
		...
		if (m.getCmd() == Message.CMD_0x20) {
			byte[] buffer = new byte[Message.CLIENT_MESSAGE_MIN_LENGTH];
			ByteBuffer.wrap(buffer).put((byte) Message.version)
					.put((byte) appid).put((byte) Message.CMD_0x20).put(uuid)
					.putChar((char) 0);
			send(buffer);
		}
	}
	...
	public abstract void onConnectionFail(String errorMsg);
