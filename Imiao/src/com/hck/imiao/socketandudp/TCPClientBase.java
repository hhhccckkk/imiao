...
public abstract class TCPClientBase implements Runnable {
	...
	public TCPClientBase(byte[] uuid, int appid, String serverAddr,
			int serverPort, int connectTimeout) throws Exception {
		if (uuid == null || uuid.length != 16) {
			throw new java.lang.IllegalArgumentException(
					"uuid byte array must be not null and lengthof 16 bytes");
		}
	...
	public void run() {
		...
		while (stoped == false) {
			try {
				...
				receiveData();
				onConnectionSuccess();
				LogUtil.D("onConnectionSuccess111111");
	...
	private boolean hasPacket() {
		if (buffer.limit() == Message.SERVER_MESSAGE_MIN_LENGTH) {
			if (buffer.hasRemaining() == true) {
				...
				int dataLen = (int) ByteBuffer.wrap(bufferArray,
						Message.SERVER_MESSAGE_MIN_LENGTH - 2, 2).getChar();
				if (dataLen == 0) {
					return true;
				} else {
